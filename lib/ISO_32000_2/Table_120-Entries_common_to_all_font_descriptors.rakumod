use v6;
role ISO_32000_2::Table_120-Entries_common_to_all_font_descriptors {
    method Type {...};
    method FontName {...};
    method FontFamily {...};
    method FontStretch {...};
...
- (Optional; PDF 1.5) A byte string specifying the preferred font family name.
EXAMPLE 1 For the font Times Bold Italic, the FontFamily is Times.

=head2 FontStretch [name]
- (Optional; PDF 1.5) The font stretch value. It is one of these names (ordered from narrowest to widest): UltraCondensed, ExtraCondensed, Condensed, SemiCondensed, Normal, SemiExpanded, Expanded, ExtraExpanded or UltraExpanded.
The specific interpretation of these values varies from font to font.
EXAMPLE 2 Condensed in one font might appear most similar to Normal in another.

=head2 FontWeight [number]
- (Optional; PDF 1.5) The weight (thickness) component of the fully-qualified font name or font specifier. If present, the value is one of 100, 200, 300, 400, 500, 600, 700, 800, or 900, where each number indicates a weight that is at least as dark as its predecessor. A value of 400 indicates a normal weight; 700 indicates bold.
The specific interpretation of these values varies from font to font.
...
=head2 StemV [number]
- (Required except for Type 3 fonts) The thickness measured horizontally, of the dominant vertical stems of glyphs in the font. Values is positive. A value of 0 indicates an unknown stem thickness.

=head2 StemH [number]
- (Optional) The thickness measured vertically of the dominant horizontal stems of glyphs in the font. Values is positive. A value of 0 indicates an unknown stem thickness. Default value: 0.
...
- (Optional; PDF 1.2) A stream containing a font program whose format is specified by the Subtype entry in the stream dictionary (see "Table 124 — Embedded font organisation for various font types").

=head2 CharSet [ASCII string or byte string]
- (Optional; meaningful only in Type 1 fonts; PDF 1.1; deprecated in PDF 2.0) A string listing the character names defined in a font subset. The names in this string is in PDF syntax — that is, each name preceded by a slash (/). The names may appear in any order. The name .notdef is omitted; it exists in the font subset. If this entry is absent, the only indication of a font subset is the subset tag in the FontName entry (see 9.9.2, "Font subsets").
