use v6;
role ISO_32000_2::Table_400-Entries_in_a_separation_dictionary {
    method Pages {...};
...
- (Required) The name of the device colourant that is used in rendering this separation, such as Cyan or PANTONE 35 CV.

=head2 ColorSpace [array]
- (Optional) An array defining a Separation or DeviceN colour space (see 8.6.6.4, "Separation colour spaces" and 8.6.6.5, "DeviceN colour spaces"). It provides additional information about the colour specified by DeviceColorant — in particular, the alternate colour space and tint transformation function that is used to represent the colourant as a process colour. This information enables an interactive PDF processor to preview the separation in a colour that approximates the device colourant.
The value of DeviceColorant matches the space’s colourant name (if it is a Separation space) or be one of the space’s colourant names (if it is a DeviceN space).
