use v6;
role ISO_32000_2::Table_5-Entries_common_to_all_stream_dictionaries {
    method Length {...};
    method Filter {...};
    method DecodeParms {...};
    method F {...};
...
- (Optional) The name, or an array of zero, one or several names, of filter(s) that is applied in processing the stream data found between the keywords stream and endstream. Multiple filters is specified in the order in which they are to be applied.
...
- (Optional; PDF 1.2) A parameter dictionary, or an array of such dictionaries, used by the filters specified by FFilter, respectively. The same rules apply as for DecodeParms.

=head2 DL [integer]
- (Optional; PDF 1.5) A non-negative integer representing the number of bytes in the decoded (defiltered) stream. This value is only a hint; for some stream filters, it may not be possible to determine this value precisely.
