use v6;
role ISO_32000_2::Table_217-Additional_entries_specific_to_a_set-OCG-state_action {
    method S {...};
    method State {...};
    method PreserveRB {...};
}
...
- (Optional) If true, indicates that radio-button state relationships between optional content groups (as specified by the RBGroups entry in the current configuration dictionary; see "Table 99 — Entries in an optional content configuration dictionary") is preserved when the states in the State array are applied. That is, if a group is set to ON (either by ON or Toggle) during processing of the State array, any other groups belonging to the same radio-button group is turned OFF. If a group is set to OFF, there is no effect on other groups.
