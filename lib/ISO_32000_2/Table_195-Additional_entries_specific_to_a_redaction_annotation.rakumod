use v6;
role ISO_32000_2::Table_195-Additional_entries_specific_to_a_redaction_annotation {
    method Subtype {...};
    method QuadPoints {...};
    method IC {...};
    method RO {...};
...
- (Optional) An array of 8 x n numbers specifying the coordinates of n quadrilaterals in default user space, as described in "Table 182 — Additional entries specific to text markup annotations" for text markup annotations. If present, these quadrilaterals denote the content region that is intended to be removed. If this entry is not present, the Rect entry denotes the content region that is intended to be removed.
...
- (Required if OverlayText is present, ignored otherwise) The appearance string that is used in formatting the overlay text when it is drawn after the affected content has been removed (see 12.7.4.3, "Variable text"). This entry is ignored if the RO entry is present.

=head2 Q [integer]
- (Optional) A code specifying the form of quadding (justification) that is used in laying out the overlay text:
0 Left-justified
1 Centred
2 Right-justified
