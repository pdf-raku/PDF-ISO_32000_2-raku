use v6;
role ISO_32000_2::Table_156-Entries_in_a_collection_sort_dictionary {
    method Type {...};
    method S {...};
    method A {...};
}
...
- (Required) The name or names of fields that the interactive PDF processor uses to sort the items in the collection. If the value is a name, it identifies a field described in the parent collection dictionary.
...
- (Optional) If the value is a boolean, it specifies whether the interactive PDF processor sorts the items in the collection in ascending order (true) or descending order (false). If the value is an array, each element of the array is a boolean value that specifies whether the entry at the same index in the S array is sorted in ascending or descending order.
