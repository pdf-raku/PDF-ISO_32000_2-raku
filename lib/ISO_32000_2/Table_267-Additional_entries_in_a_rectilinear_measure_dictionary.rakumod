use v6;
role ISO_32000_2::Table_267-Additional_entries_in_a_rectilinear_measure_dictionary {
    method R {...};
    method X {...};
    method Y {...};
    method D {...};
...
=head2 R [text string]
- (Required) A text string expressing the scale ratio of the drawing in the region corresponding to this dictionary. Universally recognised unit abbreviations is used, either matching those of the number format arrays in this dictionary or those of commonly used scale ratios.
...
The directions of the x and y axes are in the measuring coordinate system and are independent of the page rotation. These directions is determined by the BBox entry of the containing viewport (see "Table 265 — Entries in a viewport dictionary").
...
=head2 O [array]
- (Optional) An array of two numbers that specifies the origin of the measurement coordinate system in default user space coordinates. The directions by which x and y increase in value from this origin is determined by the viewport’s BBox entry (see "Table 265 — Entries in a viewport dictionary").
